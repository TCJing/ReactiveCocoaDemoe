...
public final class Signal<T, E: ErrorType> {
	...
	fileprivate let sendLock = NSLock()
...
public protocol SignalType {
	...
	func observe(_ observer: Signal<T, E>.Observer) -> Disposable?
}
...
extension SignalType {
	...
	/// itself.
	public func observeNext(_ next: (T) -> ()) -> Disposable? {
	...
	/// itself.
	public func observeCompleted(_ completed: () -> ()) -> Disposable? {
	...
	/// callback. Disposing of the Disposable will have no effect on the Signal
	/// itself.
	public func observeError(_ error: (E) -> ()) -> Disposable? {
	...
	/// callback. Disposing of the Disposable will have no effect on the Signal
	/// itself.
	public func observeInterrupted(_ interrupted: () -> ()) -> Disposable? {
		return observe(Event.sink(interrupted: interrupted))
	}

	/// Maps each value in the signal to a new value.
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	public func map<U>(_ transform: (T) -> U) -> Signal<U, E> {
	...
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	public func mapError<F>(_ transform: (E) -> F) -> Signal<T, F> {
		return Signal { observer in
			return self.observe { event in
	...
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	public func filter(_ predicate: (T) -> Bool) -> Signal<T, E> {
		return Signal { observer in
			return self.observe { event in
				switch event {
				case let .next(value):
...
extension Signal where T: SignalProducerType, E == T.E {
	...
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	public func flatten(_ strategy: FlattenStrategy) -> Signal<T.T, E> {
		switch strategy {
		case .merge:
			return signal.merge()
...
extension Signal {
	...
	/// signal will forward that error immediately.
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	public func flatMap<U>(_ strategy: FlattenStrategy, transform: (T) -> SignalProducer<U, E>) -> Signal<U, E> {
...
extension Signal where T: SignalProducerType, E == T.E {
	...
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	fileprivate func concat() -> Signal<T.T, E> {
...
private final class ConcatState<T, E: ErrorType> {
	...
	func enqueueSignalProducer(_ producer: SignalProducer<T, E>) {
	...
	func startNextSignalProducer(_ signalProducer: SignalProducer<T, E>) {
...
extension Signal where T: SignalProducerType, E == T.E {
	...
	fileprivate func merge() -> Signal<T.T, E> {
	...
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	fileprivate func switchToLatest() -> Signal<T.T, E> {
...
extension SignalType {
	...
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	public func take(_ count: Int) -> Signal<T, E> {
...
extension SignalType {
	...
	public func observeOn(_ scheduler: SchedulerType) -> Signal<T, E> {
		return Signal { observer in
			return self.observe { event in
				scheduler.schedule {
					observer(event)
...
private func observeWithStates<T, U, E>(_ signal: Signal<T, E>, _ signalState: CombineLatestState<T>, _ otherState: CombineLatestState<U>, _ lock: NSLock, _ onBothNext: () -> (), _ onError: (E) -> (), _ onBothCompleted: () -> (), _ onInterrupted: () -> ()) -> Disposable? {
...
extension SignalType {
	...
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	public func combineLatestWith<U>(_ otherSignal: Signal<U, E>) -> Signal<(T, U), E> {
		return Signal { observer in
			let lock = NSLock()
	...
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	public func delay(_ interval: TimeInterval, onScheduler scheduler: DateSchedulerType) -> Signal<T, E> {
	...
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	public func skip(_ count: Int) -> Signal<T, E> {
		precondition(count >= 0)
...
extension SignalType {
	...
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	public func sampleOn(_ sampler: Signal<(), NoError>) -> Signal<T, E> {
		return Signal { observer in
	...
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	public func takeUntil(_ trigger: Signal<(), NoError>) -> Signal<T, E> {
		return Signal { observer in
			let disposable = CompositeDisposable()
	...
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	public func combinePrevious(_ initial: T) -> Signal<(T, T), E> {
	...
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	public func reduce<U>(_ initial: U, _ combine: (U, T) -> U) -> Signal<U, E> {
	...
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	public func scan<U>(_ initial: U, _ combine: (U, T) -> U) -> Signal<U, E> {
		return Signal { observer in
...
extension SignalType {
	...
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	public func skipRepeats(_ isRepeat: (T, T) -> Bool) -> Signal<T, E> {
		return signal
			.map { Optional($0) }
			.combinePrevious(nil)
			.filter { (a, b) in
	...
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	public func skipWhile(_ predicate: (T) -> Bool) -> Signal<T, E> {
		return Signal { observer in
			var shouldSkip = true
	...
	/// already.
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	public func takeUntilReplacement(_ replacement: Signal<T, E>) -> Signal<T, E> {
		return Signal { observer in
			let disposable = CompositeDisposable()
	...
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	public func takeLast(_ count: Int) -> Signal<T, E> {
		return Signal { observer in
			var buffer = [T]()
	...
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	public func takeWhile(_ predicate: (T) -> Bool) -> Signal<T, E> {
		return Signal { observer in
			return self.observe { event in
				switch event {
				case let .next(value):
...
extension SignalType {
	...
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	public func zipWith<U>(_ otherSignal: Signal<U, E>) -> Signal<(T, U), E> {
		return Signal { observer in
			...
			let flush = { () -> () in
				...
				states.modify { states in
					...
					updatedStates.0.values.removeSubrange(0 ..< extractCount)
					updatedStates.1.values.removeSubrange(0 ..< extractCount)
					return updatedStates
				}
				
				while !originalStates.0.values.isEmpty && !originalStates.1.values.isEmpty {
					let left = originalStates.0.values.remove(at: 0)
					let right = originalStates.1.values.remove(at: 0)
	...
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	public func attempt(_ operation: (T) -> Result<(), E>) -> Signal<T, E> {
	...
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	public func attemptMap<U>(_ operation: (T) -> Result<U, E>) -> Signal<U, E> {
		return Signal { observer in
	...
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	public func throttle(_ interval: TimeInterval, onScheduler scheduler: DateSchedulerType) -> Signal<T, E> {
		precondition(interval >= 0)
...
@warn_unused_result(message="Did you forget to call `observe` on the signal?")
public func combineLatest<S: Sequence, T, Error where S.Iterator.Element == Signal<T, Error>>(_ signals: S) -> Signal<[T], Error> {
...
@warn_unused_result(message="Did you forget to call `observe` on the signal?")
public func zip<S: Sequence, T, Error where S.Iterator.Element == Signal<T, Error>>(_ signals: S) -> Signal<[T], Error> {
	var generator = signals.makeIterator()
	if let first = generator.next() {
		let initial = first.map { [$0] }
		return IteratorSequence(generator).reduce(initial) { signal, next in
...
extension SignalType {
	...
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	public func timeoutWithError(_ error: E, afterInterval interval: TimeInterval, onScheduler scheduler: DateSchedulerType) -> Signal<T, E> {
		precondition(interval >= 0)
...
extension SignalType where E: NoError {
	...
	@warn_unused_result(message="Did you forget to call `observe` on the signal?")
	public func promoteErrors<F: ErrorProtocol>(_: F.Type) -> Signal<T, F> {
		return Signal { observer in
			return self.observe { event in
				switch event {
				case let .next(value):
