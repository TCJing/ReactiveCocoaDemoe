...
public struct SignalProducer<T, E: ErrorType> {
	...
	fileprivate let startHandler: (Signal<T, E>.Observer, CompositeDisposable) -> ()
	...
	public init<S: Sequence where S.Iterator.Element == T>(values: S) {
		self.init({ observer, disposable in
			for value in values {
				sendNext(observer, value)
	...
	/// occurred.
	public static func attempt(_ operation: () -> Result<T, E>) -> SignalProducer {
		return self.init { observer, disposable in
	...
	/// `Interrupted` event.
	public func startWithSignal(@noescape _ setUp: (Signal<T, E>, Disposable) -> ()) {
...
private struct BufferState<T, Error: ErrorType> {
	...
	// if necessary.
	mutating func addValue(_ value: T, upToCapacity capacity: Int) {
		values.append(value)
...
public protocol SignalProducerType {
	...
	/// Creates a Signal from the producer, passes it into the given closure,
	/// then starts sending events on the Signal when the closure has returned.
	func startWithSignal(@noescape _ setUp: (Signal<T, E>, Disposable) -> ())
}
...
extension SignalProducerType {
	...
	public func start(_ sink: Event<T, E>.Sink = Event<T, E>.sink()) -> Disposable {
	...
	public func startWithNext(_ next: (T) -> ()) -> Disposable {
	...
	/// Returns a Disposable which can be used to interrupt the work associated
	/// with the Signal.
	public func startWithCompleted(_ completed: () -> ()) -> Disposable {
	...
	/// Returns a Disposable which can be used to interrupt the work associated
	/// with the Signal.
	public func startWithError(_ error: (E) -> ()) -> Disposable {
	...
	/// Returns a Disposable which can be used to interrupt the work associated
	/// with the Signal.
	public func startWithInterrupted(_ interrupted: () -> ()) -> Disposable {
	...
	public func lift<U, F>(_ transform: (Signal<T, E>) -> Signal<U, F>) -> SignalProducer<U, F> {
	...
	/// yielded from start().
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func lift<U, F, V, G>(_ transform: (Signal<U, F>) -> (Signal<T, E>) -> Signal<V, G>) -> (SignalProducer<U, F>) -> SignalProducer<V, G> {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func map<U>(_ transform: (T) -> U) -> SignalProducer<U, E> {
		return lift { $0.map(transform) }
	}

	/// Maps errors in the producer to a new error.
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func mapError<F>(_ transform: (E) -> F) -> SignalProducer<T, F> {
		return lift { $0.mapError(transform) }
	}

	/// Preserves only the values of the producer that pass the given predicate.
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func filter(_ predicate: (T) -> Bool) -> SignalProducer<T, E> {
	...
	/// input producer.
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func take(_ count: Int) -> SignalProducer<T, E> {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func observeOn(_ scheduler: SchedulerType) -> SignalProducer<T, E> {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func combineLatestWith<U>(_ otherProducer: SignalProducer<U, E>) -> SignalProducer<(T, U), E> {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func delay(_ interval: TimeInterval, onScheduler scheduler: DateSchedulerType) -> SignalProducer<T, E> {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func skip(_ count: Int) -> SignalProducer<T, E> {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func sampleOn(_ sampler: SignalProducer<(), NoError>) -> SignalProducer<T, E> {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func takeUntil(_ trigger: SignalProducer<(), NoError>) -> SignalProducer<T, E> {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func combinePrevious(_ initial: T) -> SignalProducer<(T, T), E> {
		return lift { $0.combinePrevious(initial) }
	}

	/// Like `scan`, but sends only the final value and then immediately completes.
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func reduce<U>(_ initial: U, _ combine: (U, T) -> U) -> SignalProducer<U, E> {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func scan<U>(_ initial: U, _ combine: (U, T) -> U) -> SignalProducer<U, E> {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func skipRepeats(_ isRepeat: (T, T) -> Bool) -> SignalProducer<T, E> {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func skipWhile(_ predicate: (T) -> Bool) -> SignalProducer<T, E> {
	...
	/// already.
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func takeUntilReplacement(_ replacement: SignalProducer<T, E>) -> SignalProducer<T, E> {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func takeLast(_ count: Int) -> SignalProducer<T, E> {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func takeWhile(_ predicate: (T) -> Bool) -> SignalProducer<T, E> {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func zipWith<U>(_ otherProducer: SignalProducer<U, E>) -> SignalProducer<(T, U), E> {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func attempt(_ operation: (T) -> Result<(), E>) -> SignalProducer<T, E> {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func attemptMap<U>(_ operation: (T) -> Result<U, E>) -> SignalProducer<U, E> {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func throttle(_ interval: TimeInterval, onScheduler scheduler: DateSchedulerType) -> SignalProducer<T, E> {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func timeoutWithError(_ error: E, afterInterval interval: TimeInterval, onScheduler scheduler: DateSchedulerType) -> SignalProducer<T, E> {
...
extension SignalProducerType where E: NoError {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func promoteErrors<F: ErrorProtocol>(_: F.Type) -> SignalProducer<T, F> {
...
extension SignalProducerType {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func on(started: (() -> ())? = nil, event: ((Event<T, E>) -> ())? = nil, error: ((E) -> ())? = nil, completed: (() -> ())? = nil, interrupted: (() -> ())? = nil, terminated: (() -> ())? = nil, disposed: (() -> ())? = nil, next: ((T) -> ())? = nil) -> SignalProducer<T, E> {
		return SignalProducer { observer, compositeDisposable in
			started?()
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func startOn(_ scheduler: SchedulerType) -> SignalProducer<T, E> {
		return SignalProducer { observer, compositeDisposable in
...
@warn_unused_result(message="Did you forget to call `start` on the producer?")
public func combineLatest<S: Sequence, T, Error where S.Iterator.Element == SignalProducer<T, Error>>(_ producers: S) -> SignalProducer<[T], Error> {
...
@warn_unused_result(message="Did you forget to call `start` on the producer?")
public func zip<S: Sequence, T, Error where S.Iterator.Element == SignalProducer<T, Error>>(_ producers: S) -> SignalProducer<[T], Error> {
	var generator = producers.makeIterator()
	if let first = generator.next() {
		let initial = first.map { [$0] }
		return IteratorSequence(generator).reduce(initial) { producer, next in
...
extension SignalProducerType {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func flatMapError<F>(_ handler: (E) -> SignalProducer<T, F>) -> SignalProducer<T, F> {
...
extension SignalProducer {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func concat(_ next: SignalProducer) -> SignalProducer {
...
extension SignalProducerType {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func times(_ count: Int) -> SignalProducer<T, E> {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func retry(_ count: Int) -> SignalProducer<T, E> {
		precondition(count >= 0)
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func then<U>(_ replacement: SignalProducer<U, E>) -> SignalProducer<U, E> {
...
extension SignalProducer where T: SignalProducerType, E == T.E {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func flatten(_ strategy: FlattenStrategy) -> SignalProducer<T.T, E> {
...
extension SignalProducer {
	...
	/// producer will forward that error immediately.
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	public func flatMap<U>(_ strategy: FlattenStrategy, transform: (T) -> SignalProducer<U, E>) -> SignalProducer<U, E> {
...
private func combineLatestWith<T, U, E>(_ otherSignal: Signal<U, E>) -> (Signal<T, E>) -> Signal<(T, U), E> {
	return { $0.combineLatestWith(otherSignal) }
}

private func zipWith<T, U, E>(_ otherSignal: Signal<U, E>) -> (Signal<T, E>) -> Signal<(T, U), E> {
	return { $0.zipWith(otherSignal) }
}

private func sampleOn<T, E>(_ sampler: Signal<(), NoError>) -> (Signal<T, E>) -> Signal<T, E> {
	return { $0.sampleOn(sampler) }
}

private func takeUntil<T, E>(_ trigger: Signal<(), NoError>) -> (Signal<T, E>) -> Signal<T, E> {
	return { $0.takeUntil(trigger) }
}

private func takeUntilReplacement<T, E>(_ replacement: Signal<T, E>) -> (Signal<T, E>) -> Signal<T, E> {
